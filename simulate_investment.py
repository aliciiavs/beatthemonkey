...
def simulate_investment2(stock, initial_amount, monthly_investment, strategy, years, preset_id=None):
    # Ensure the index is datetime
    yesterday = pd.Timestamp.today().normalize() - pd.DateOffset(days=1)
    earliest = yesterday - pd.DateOffset(years=years)
    ...
    if strategy in ["first", "random", "losing", "buythedip"]:
        for date in data.index:
            ...
            if strategy == "first":
                ...
                recent_peak = float(first_price)  # Initial price
                for date in data.index:
                    current_price = float(data.loc[date])  # Get the current price
                    
                    # Buy if the price drops by 5% or more from the recent peak
                    if current_price < recent_peak * 0.95:
                        shares += monthly_investment / current_price  # Buy shares at the dip
                        total_investment += monthly_investment  # Add to total invested
                        recent_peak = current_price  # Reset peak price to current price
                        
                    # Sell if the price increases by 10% or more from the recent peak
                    if current_price >= recent_peak * 1.1 and shares > 0:
                        # Realize profits by selling all shares
                        total_investment += (shares * current_price)  # Add the proceeds from the sale to total investment
                        shares = 0  # After selling, you have no shares left

                        # Reinvest the proceeds by buying shares again at the current price
                        shares += total_investment / current_price  # Buy new shares with the money
                        total_investment = shares * current_price  # Update the total investment

                    # Track portfolio value and total investment
                    portfolio_values.at[date, 'Portfolio Value'] = shares * current_price
                    portfolio_values.at[date, 'Total Invested'] = total_investment
